...
function diffStrings(a3, b, options) {
  ...
  if (truncate3) {
    ...
    const _a2 = aMultipleLines ? `${a3.split(aNewLineSymbol, truncateThreshold).join(aNewLineSymbol)}
` : a3;
...
// src/tests/e2e/appointmentRoutes.test.ts
...
`);
  ...
  existingAppointments.forEach((appointment, index2) => {
    console.log(
      `${index2 + 1}. ${(0, import_moment_timezone2.default)(appointment.startTime).tz(TIMEZONE2).format("HH:mm")} - ${(0, import_moment_timezone2.default)(appointment.endTime).tz(TIMEZONE2).format("HH:mm")} (${appointment.status})`
    );
  });
  console.log("==========================================");
  const slots = [];
...
\u{1F4C5} CONFIGURA\xC7\xC3O DE HOR\xC1RIOS:`);
  ...
  const now3 = (0, import_moment_timezone2.default)().tz(TIMEZONE2);
  const isToday = requestedDate.isSame(now3, "day");
  console.log(`
\u23F0 VERIFICA\xC7\xC3O DE HOR\xC1RIO:`);
  console.log(`   \xC9 hoje? ${isToday}`);
  console.log(`   Hor\xE1rio atual: ${now3.format("HH:mm")}`);
  console.log(`   Primeiro slot calculado: ${currentSlot.format("HH:mm")}`);
  if (isToday && currentSlot.isBefore(now3)) {
    const currentHour = now3.hour();
    const currentMinute = now3.minute();
    if (currentMinute > 0) {
      currentSlot = now3.clone().add(1, "hour").startOf("hour");
    } else {
      currentSlot = now3.clone().startOf("hour");
    }
...
\u{1F504} INICIANDO LOOP DE GERA\xC7\xC3O DE SLOTS:`);
  ...
  while (currentSlot.isBefore(endTime)) {
   ...
   \u{1F4CD} Slot ${slotCount}: ${currentSlot.format(
    ...
    const isPastSlot = isToday && currentSlot.isBefore(now3);
    if (isPastSlot) {
...
async function updateAppointmentStatus(appointmentId, status, userId, userRole) {
  ...
  if (status === "cancelled") {
    ...
    const hoursUntilAppointment = appointmentTime.diff(now3, "hours");
...
async function createTestApp() {
  ...
  const authenticateMiddleware = async (request, reply) => {
    try {
      ...
      throw new Unauthorized("Token inv\xE1lido");
    }
  };
  app.post(
    "/appointments",
...
describe("Appointment Routes E2E", () => {
  ...
  describe("POST /availability", () => {
    it("deve permitir m\xE9dico criar disponibilidade", async () => {
      ...
      globalExpect(response.statusCode).toBe(201);
      const body = JSON.parse(response.body);
      globalExpect(body.status).toBe("success");
      globalExpect(body.data.dayOfWeek).toBe(1);
      globalExpect(body.data.startTime).toBe("08:00");
      globalExpect(body.data.endTime).toBe("17:00");
    });
    it("deve impedir paciente de criar disponibilidade", async () => {
      ...
      globalExpect(response.statusCode).toBe(403);
      const body = JSON.parse(response.body);
      globalExpect(body.status).toBe("error");
    });
  });
  describe("GET /availability/:doctorId", () => {
    it("deve retornar disponibilidade do m\xE9dico", async () => {
      ...
      globalExpect(response.statusCode).toBe(200);
      const body = JSON.parse(response.body);
      globalExpect(body.status).toBe("success");
      globalExpect(body.data).toHaveLength(1);
      globalExpect(body.data[0].dayOfWeek).toBe(1);
    });
    it("deve retornar array vazio se m\xE9dico n\xE3o tiver disponibilidade", async () => {
      const response = await app.inject({
        method: "GET",
        url: `/availability/${doctor.id}`
      });
      ...
      const body = JSON.parse(response.body);
      globalExpect(body.status).toBe("success");
      globalExpect(body.data).toHaveLength(0);
    });
  });
  describe("GET /appointments/available-slots", () => {
    it("deve retornar slots dispon\xEDveis para m\xE9dico com disponibilidade", async () => {
      await prisma.availability.create({
        data: {
          doctorId: doctor.id,
          dayOfWeek: 1,
      ...
      const response = await app.inject({
        ...
        query: {
          doctorId: doctor.id,
          date: "2024-01-01"
          // Assumindo que seja uma segunda-feira
        }
      });
      globalExpect(response.statusCode).toBe(200);
      const body = JSON.parse(response.body);
      globalExpect(body.status).toBe("success");
      globalExpect(Array.isArray(body.data)).toBe(true);
    });
    it("deve retornar slots vazios se m\xE9dico n\xE3o tiver disponibilidade", async () => {
      const response = await app.inject({
        method: "GET",
        url: "/appointments/available-slots",
        query: {
  ...
  describe("POST /appointments", () => {
    ...
    it("deve permitir paciente criar agendamento", async () => {
      ...
      const body = JSON.parse(response.body);
      globalExpect(body.status).toBe("success");
      globalExpect(body.data.patientId).toBe(patient.id);
      globalExpect(body.data.doctorId).toBe(doctor.id);
      globalExpect(body.data.notes).toBe("Consulta de rotina");
    });
    it("deve impedir m\xE9dico de agendar para si mesmo", async () => {
      const appointmentData = {
        doctorId: doctor.id,
        startTime: "2024-12-02T10:00:00.000Z",
        notes: "Teste"
      ...
      globalExpect(response.statusCode).toBe(400);
      const body = JSON.parse(response.body);
      globalExpect(body.status).toBe("error");
    });
    it("deve retornar erro sem autentica\xE7\xE3o", async () => {
      const appointmentData = {
        doctorId: doctor.id,
        startTime: "2024-12-02T10:00:00.000Z"
      };
      ...
      globalExpect(response.statusCode).toBe(401);
    });
  });
  describe("GET /appointments/my", () => {
    beforeEach(async () => {
      await prisma.appointment.create({
        data: {
          ...
          doctorId: doctor.id,
          startTime: /* @__PURE__ */ new Date("2024-12-02T10:00:00.000Z"),
          endTime: /* @__PURE__ */ new Date("2024-12-02T10:50:00.000Z"),
          status: "scheduled",
    ...
    it("deve retornar agendamentos do paciente", async () => {
      ...
      globalExpect(body.status).toBe("success");
      globalExpect(body.data).toHaveLength(1);
      globalExpect(body.data[0].patientId).toBe(patient.id);
    });
    it("deve retornar agendamentos do m\xE9dico", async () => {
      ...
      globalExpect(body.status).toBe("success");
      globalExpect(body.data).toHaveLength(1);
      globalExpect(body.data[0].doctorId).toBe(doctor.id);
    });
    it("deve filtrar agendamentos por status", async () => {
      ...
      const body = JSON.parse(response.body);
      globalExpect(body.status).toBe("success");
      globalExpect(body.data[0].status).toBe("scheduled");
    });
  });
  describe("PUT /appointments/:id/status", () => {
    ...
    it("deve permitir paciente confirmar agendamento", async () => {
      ...
      const body = JSON.parse(response.body);
      globalExpect(body.status).toBe("success");
      globalExpect(body.data.status).toBe("confirmed");
    });
    it("deve permitir m\xE9dico atualizar status", async () => {
      const response = await app.inject({
        method: "PUT",
        url: `/appointments/${appointment.id}/status`,
        headers: {
      ...
      const body = JSON.parse(response.body);
      globalExpect(body.status).toBe("success");
      globalExpect(body.data.status).toBe("completed");
    });
    it("deve retornar erro para agendamento inexistente", async () => {
      ...
      globalExpect(response.statusCode).toBe(404);
    });
  });
  describe("GET /appointments/today", () => {
    beforeEach(async () => {
      ...
      await prisma.appointment.create({
        data: {
          ...
          doctorId: doctor.id,
          startTime: today,
          endTime: new Date(today.getTime() + 50 * 60 * 1e3),
          // +50 minutos
          status: "scheduled",
    ...
    it("deve retornar agendamentos do dia para m\xE9dico", async () => {
      const response = await app.inject({
        method: "GET",
