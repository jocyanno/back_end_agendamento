...
describe("Auth Middleware", () => {
  ...
  beforeEach(() => {
    mockRequest = {
      jwtVerify: vi.fn().mockResolvedValue(void 0),
  ...
  it("deve autenticar token v\xE1lido", async () => {
    await autenticarToken(
      mockRequest,
      mockReply
    );
    globalExpect(mockRequest.jwtVerify).toHaveBeenCalled();
    globalExpect(mockRequest.usuario).toEqual({
      id: "user-id",
      register: "patient"
    });
  });
  it("deve falhar com token inv\xE1lido", async () => {
    vi.mocked(mockRequest.jwtVerify).mockRejectedValue({
      code: "FST_JWT_AUTHORIZATION_TOKEN_INVALID"
    });
    await globalExpect(
      autenticarToken(mockRequest, mockReply)
    ).rejects.toThrow("Token de autentica\xE7\xE3o inv\xE1lido");
  });
  it("deve falhar sem header Authorization", async () => {
    mockRequest.headers = {};
    await globalExpect(
      autenticarToken(mockRequest, mockReply)
    ).rejects.toThrow("Token de autentica\xE7\xE3o n\xE3o fornecido");
  });
  it("deve falhar com formato de token incorreto", async () => {
    ...
    await globalExpect(
      autenticarToken(mockRequest, mockReply)
    ).rejects.toThrow("Formato de token inv\xE1lido. Use: Bearer <token>");
  });
  it("deve falhar com token expirado", async () => {
    ...
    await globalExpect(
      autenticarToken(mockRequest, mockReply)
    ).rejects.toThrow("Token de autentica\xE7\xE3o expirado");
  });
});
