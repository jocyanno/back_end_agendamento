...
// src/tests/unit/usuarioService.test.ts
...
vi.mock("moment-timezone", () => ({
  default: vi.fn(() => ({
    isValid: () => true,
    toDate: () => /* @__PURE__ */ new Date("2024-01-01")
  }))
}));
describe("usuarioService", () => {
  ...
  describe("authenticateUser", () => {
    it("deve autenticar usu\xE1rio com credenciais v\xE1lidas", async () => {
      const mockUser = {
        id: "1",
        email: "test@test.com",
        password: "hashedPassword",
      ...
      vi.mocked(import_bcrypt2.default.compare).mockResolvedValue(true);
      ...
      globalExpect(result).toEqual({
        token: "mock-token",
        usuario: globalExpect.objectContaining({
          id: "1",
          email: "test@test.com",
          register: "patient"
        })
      });
      globalExpect(result.usuario).not.toHaveProperty("password");
    });
    it("deve lan\xE7ar erro com credenciais inv\xE1lidas", async () => {
      vi.mocked(prisma.users.findUnique).mockResolvedValue(null);
      await globalExpect(
        authenticateUser("test@test.com", "password", {})
      ).rejects.toThrow(Unauthorized);
    });
    it("deve lan\xE7ar erro com senha incorreta", async () => {
      const mockUser = {
        id: "1",
        email: "test@test.com",
        password: "hashedPassword",
      ...
      vi.mocked(prisma.users.findUnique).mockResolvedValue(mockUser);
      vi.mocked(import_bcrypt2.default.compare).mockResolvedValue(false);
      await globalExpect(
  ...
  describe("createUser", () => {
    it("deve criar usu\xE1rio com dados v\xE1lidos", async () => {
      const userData = {
        email: "test@test.com",
      ...
      vi.mocked(import_bcrypt2.default.hash).mockResolvedValue("hashedPassword");
      ...
      globalExpect(import_bcrypt2.default.hash).toHaveBeenCalledWith("password123", 10);
    });
    it("deve impedir cria\xE7\xE3o de usu\xE1rio com register doctor", async () => {
      const userData = {
      ...
      await globalExpect(createUser(userData)).rejects.toThrow(BadRequest);
    });
  });
  describe("createUserAdmin", () => {
    it("deve criar usu\xE1rio admin com qualquer register", async () => {
      const userData = {
        email: "doctor@test.com",
        password: "password123",
        cpf: "12345678901",
  ...
  describe("updateUser", () => {
    it("deve atualizar usu\xE1rio existente", async () => {
      const userId = "1";
      const updateData = {
        name: "Updated Name",
    ...
    it("deve lan\xE7ar erro se usu\xE1rio n\xE3o existir", async () => {
      vi.mocked(prisma.users.findUnique).mockResolvedValue(null);
      await globalExpect(
  ...
  describe("deleteUser", () => {
    it("deve deletar usu\xE1rio existente", async () => {
      const userId = "1";
      const adminId = "2";
      const mockUser = {
        id: "1",
      ...
      globalExpect(result).toEqual({ message: "User deleted successfully" });
      globalExpect(prisma.users.delete).toHaveBeenCalledWith({
        where: { id: userId }
      });
    });
    it("deve impedir admin de deletar a si mesmo", async () => {
      ...
      vi.mocked(prisma.users.findUnique).mockResolvedValue(mockUser);
      await globalExpect(deleteUser(userId, adminId)).rejects.toThrow(BadRequest);
    });
    it("deve lan\xE7ar erro se usu\xE1rio n\xE3o existir", async () => {
      vi.mocked(prisma.users.findUnique).mockResolvedValue(null);
      await globalExpect(deleteUser("inexistent-id", "admin-id")).rejects.toThrow(
        NotFound
      );
    });
  });
  describe("searchUsuario", () => {
    it("deve encontrar usu\xE1rio por ID", async () => {
      const mockUser = {
        id: "1",
        email: "test@test.com",
        password: "hashedPassword",
      ...
      globalExpect(result).toEqual(mockUser);
    });
    it("deve lan\xE7ar erro se usu\xE1rio n\xE3o existir", async () => {
      vi.mocked(prisma.users.findUnique).mockResolvedValue(null);
      await globalExpect(searchUsuario("inexistent-id")).rejects.toThrow(NotFound);
    });
  });
});
