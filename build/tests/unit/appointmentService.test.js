...
// src/tests/unit/appointmentService.test.ts
vi.mock("@/lib/prisma", () => ({
  prisma: {
    appointment: {
      findFirst: vi.fn(),
...
vi.mock("moment-timezone", () => ({
  default: vi.fn(() => ({
    ...
    toDate: vi.fn(() => /* @__PURE__ */ new Date("2024-01-01T08:00:00.000Z")),
...
describe("appointmentService", () => {
  ...
  describe("checkSlotAvailability", () => {
    it("deve permitir agendamento em hor\xE1rio livre", async () => {
      ...
      await globalExpect(
        checkSlotAvailability("doctor-id", /* @__PURE__ */ new Date(), /* @__PURE__ */ new Date())
      ).resolves.not.toThrow();
    });
    it("deve lan\xE7ar erro se hor\xE1rio estiver ocupado", async () => {
      vi.mocked(prisma.availability.findFirst).mockResolvedValue({
        id: "availability-id",
        doctorId: "doctor-id",
        dayOfWeek: 1,
  ...
  describe("generateAvailableSlots", () => {
    it("deve retornar slots vazios se m\xE9dico n\xE3o tiver disponibilidade", async () => {
      ...
      globalExpect(slots).toEqual([]);
    });
    it("deve gerar slots dispon\xEDveis corretamente", async () => {
      vi.mocked(prisma.availability.findFirst).mockResolvedValue({
        id: "availability-id",
        doctorId: "doctor-id",
        dayOfWeek: 1,
      ...
      const slots = await generateAvailableSlots("doctor-id", "2024-01-01");
      globalExpect(Array.isArray(slots)).toBe(true);
    });
  });
  describe("createAppointment", () => {
    it("deve criar agendamento com dados v\xE1lidos", async () => {
      const mockPatient = {
        id: "patient-id",
      ...
      vi.mocked(prisma.users.findUnique).mockResolvedValueOnce(mockPatient).mockResolvedValueOnce(mockDoctor);
      vi.mocked(prisma.availability.findFirst).mockResolvedValue({
        id: "availability-id",
        doctorId: "doctor-id",
        dayOfWeek: 1,
      ...
      globalExpect(result).toEqual(mockAppointment);
    });
    it("deve lan\xE7ar erro se paciente n\xE3o existir", async () => {
      ...
      ).rejects.toThrow("Paciente n\xE3o encontrado");
    });
    it("deve lan\xE7ar erro se m\xE9dico n\xE3o existir", async () => {
      const mockPatient = {
        id: "patient-id",
        email: "patient@test.com",
        register: "patient"
      };
      vi.mocked(prisma.users.findUnique).mockResolvedValueOnce(mockPatient).mockResolvedValueOnce(null);
      await globalExpect(
        createAppointment({
          patientId: "patient-id",
      ...
      ).rejects.toThrow("M\xE9dico n\xE3o encontrado");
    });
  });
  ...
  describe("updateAppointmentStatus", () => {
    it("deve atualizar status do agendamento", async () => {
      const mockAppointment = {
        ...
        patientId: "patient-id",
        doctorId: "doctor-id",
        startTime: /* @__PURE__ */ new Date("2024-12-01T10:00:00.000Z"),
        endTime: /* @__PURE__ */ new Date("2024-12-01T11:00:00.000Z"),
        status: "scheduled",
      ...
      globalExpect(result).toEqual(updatedAppointment);
    });
    it("deve lan\xE7ar erro se agendamento n\xE3o existir", async () => {
    ...
    it("deve lan\xE7ar erro se usu\xE1rio n\xE3o tiver permiss\xE3o", async () => {
      const mockAppointment = {
        id: "appointment-id",
    ...
    it("deve lan\xE7ar erro ao tentar alterar agendamento finalizado", async () => {
      const mockAppointment = {
        id: "appointment-id",
        patientId: "patient-id",
        doctorId: "doctor-id",
  ...
  describe("getAppointmentById", () => {
    it("deve retornar agendamento por ID", async () => {
      ...
      globalExpect(result?.startTime).toBe("2024-01-01T08:00:00.000Z");
      globalExpect(result?.endTime).toBe("2024-01-01T08:00:00.000Z");
    });
    it("deve retornar null se agendamento n\xE3o existir", async () => {
      vi.mocked(prisma.appointment.findUnique).mockResolvedValue(null);
