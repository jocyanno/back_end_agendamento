...
var mockAppointment = {
  ...
  patientId: "patient-1",
  doctorId: "doctor-1",
  startTime: /* @__PURE__ */ new Date("2024-01-15T10:00:00Z"),
  endTime: /* @__PURE__ */ new Date("2024-01-15T10:50:00Z"),
  status: "scheduled",
...
describe("Notification Service", () => {
  ...
  describe("sendEmail", () => {
    it("deve enviar email com sucesso", async () => {
      ...
      globalExpect(sendEmail).toHaveBeenCalledWith(
        "test@test.com",
        "Teste",
        "<p>Conte\xFAdo</p>"
      );
  ...
  describe("getAppointmentConfirmationTemplate", () => {
    it("deve gerar template de confirma\xE7\xE3o correto", () => {
      const data = {
        patientName: "Jo\xE3o Silva",
        doctorName: "Dr. Maria",
        date: "15/01/2024",
        time: "10:00"
      };
      const template = getAppointmentConfirmationTemplate(data);
      globalExpect(template).toContain("Confirma\xE7\xE3o de Agendamento");
      globalExpect(template).toContain("Jo\xE3o Silva");
      globalExpect(template).toContain("Dr. Maria");
      globalExpect(template).toContain("15/01/2024");
      globalExpect(template).toContain("10:00");
      globalExpect(template).toContain(
        "O evento foi adicionado ao seu calend\xE1rio Google"
      );
    });
    it("deve gerar template com informa\xE7\xF5es completas", () => {
      const data = {
        patientName: "Jo\xE3o Silva",
        doctorName: "Dr. Maria",
        date: "15/01/2024",
        time: "10:00",
        location: "Cl\xEDnica ABC"
      };
      const template = getAppointmentConfirmationTemplate(data);
      globalExpect(template).toContain("Confirma\xE7\xE3o de Agendamento");
      globalExpect(template).toContain("Cl\xEDnica ABC");
      globalExpect(template).toContain(
        "O evento foi adicionado ao seu calend\xE1rio Google"
      );
    });
  ...
  describe("getAppointmentCancellationTemplate", () => {
    it("deve gerar template de cancelamento correto", () => {
      ...
      globalExpect(template).toContain("Agendamento Cancelado");
      globalExpect(template).toContain("Jo\xE3o Silva");
      globalExpect(template).toContain("Dr. Maria");
      globalExpect(template).toContain("15/01/2024");
      globalExpect(template).toContain("10:00");
      globalExpect(template).toContain("Conflito de agenda");
    });
    it("deve gerar template sem motivo", () => {
      ...
      const template = getAppointmentCancellationTemplate(data);
      globalExpect(template).toContain("Agendamento Cancelado");
      globalExpect(template).not.toContain("Motivo:");
    });
  });
  describe("createNotification", () => {
    it("deve criar notifica\xE7\xE3o", async () => {
      ...
      globalExpect(createNotification).toHaveBeenCalledWith(notificationData);
      globalExpect(result).toHaveProperty("id");
    });
  });
  describe("sendAppointmentConfirmation", () => {
    it("deve enviar confirma\xE7\xE3o de agendamento", async () => {
      vi.mocked(sendAppointmentConfirmation).mockResolvedValue(void 0);
      await sendAppointmentConfirmation(mockAppointment);
      globalExpect(sendAppointmentConfirmation).toHaveBeenCalledWith(mockAppointment);
    });
  });
  describe("sendAppointmentCancellation", () => {
    it("deve enviar notifica\xE7\xE3o de cancelamento", async () => {
      vi.mocked(sendAppointmentCancellation).mockResolvedValue(void 0);
      await sendAppointmentCancellation(mockAppointment, "Conflito de agenda");
      globalExpect(sendAppointmentCancellation).toHaveBeenCalledWith(
        mockAppointment,
        "Conflito de agenda"
      );
    });
    it("deve enviar notifica\xE7\xE3o de cancelamento sem motivo", async () => {
      vi.mocked(sendAppointmentCancellation).mockResolvedValue(void 0);
      await sendAppointmentCancellation(mockAppointment);
      globalExpect(sendAppointmentCancellation).toHaveBeenCalledWith(mockAppointment);
    });
  });
});
